...
func (api *Client) SetUserPhotoContext(ctx context.Context, image string, params UserSetPhotoParams) (err error) {
	...
	err = postLocalWithMultipartResponse(ctx, api.httpclient, api.endpoint+"users.setPhoto", image, "image", api.token, values, api.cookies, response, api)
	if err != nil {
		return err
	}
...
func (api *Client) SetUserCustomFieldsContext(ctx context.Context, userID string, customFields map[string]UserProfileCustomField) error {
	...
	response := &userResponseFull{}
	if _, err := postForm(ctx, api.httpclient, APIURL+"users.profile.set", values, response, api, api.cookies); err != nil {
		return err
	}
