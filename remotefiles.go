...
func (api *Client) AddRemoteFileContext(ctx context.Context, params RemoteFileParameters) (remotefile *RemoteFile, err error) {
	...
	if params.PreviewImage != "" {
		...
		err = postWithMultipartResponse(ctx, api.httpclient, api.endpoint+"files.remote.add", name, "preview_image", api.token, values, api.cookies, params.PreviewImageReader, response, api)
	} else {
...
func (api *Client) UpdateRemoteFileContext(ctx context.Context, fileID string, params RemoteFileParameters) (remotefile *RemoteFile, err error) {
	...
	if params.PreviewImage != "" {
		err = postLocalWithMultipartResponse(ctx, api.httpclient, api.endpoint+"files.remote.update", params.PreviewImage, "preview_image", api.token, values, api.cookies, response, api)
	} else if params.PreviewImageReader != nil {
		name := params.PreviewImageName
		if name == "" {
			name = "preview.png"
		}
		err = postWithMultipartResponse(ctx, api.httpclient, api.endpoint+"files.remote.update", name, "preview_image", api.token, values, api.cookies, params.PreviewImageReader, response, api)
	} else {
