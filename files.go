...
func (api *Client) UploadFileContext(ctx context.Context, params FileUploadParameters) (file *File, err error) {
	...
	if params.Content != "" {
		...
		err = postLocalWithMultipartResponse(ctx, api.httpclient, api.endpoint+"files.upload", params.File, "file", api.token, values, api.cookies, response, api)
		...
		err = postWithMultipartResponse(ctx, api.httpclient, api.endpoint+"files.upload", params.Filename, "file", api.token, values, api.cookies, params.Reader, response, api)
	}
...
func (api *Client) UploadToURLB(ctx context.Context, fu *FileUploadURL, mimeType string, data []byte) error {
...
func (api *Client) UploadToURL(ctx context.Context, params UploadToURLParameters) (err error) {
	...
	if params.Content != "" {
		...
		err = postWithMultipartResponse(ctx, api.httpclient, params.UploadURL, params.Filename, "file", api.token, values, api.cookies, contentReader, nil, api)
	} else if params.File != "" {
		err = postLocalWithMultipartResponse(ctx, api.httpclient, params.UploadURL, params.File, "file", api.token, values, api.cookies, nil, api)
	} else if params.Reader != nil {
		err = postWithMultipartResponse(ctx, api.httpclient, params.UploadURL, params.Filename, "file", api.token, values, api.cookies, params.Reader, nil, api)
	}
	return err
}
